...
var driverErrorMessage =
  "Please provide a driver (as returned" +
...
function getCounts(driver, prev) {
  return getLastCounts(driver, prev).then(function(lastCounts) {
    return driver.executeScript("gc()").then(function() {
      if (!prev) {
        ...
        lastCounts.counts.jsHeapSizeUsed < prev.counts.jsHeapSizeUsed
      ) {
        console.log("heap is still decreazing - requerying after delay", [
          lastCounts.counts.jsHeapSizeUsed,
          prev.counts.jsHeapSizeUsed
        ]);
...
function getLastCounts(driver, last) {
  ...
  return driver
    ...
    .then(function(performanceLogs) {
      last.gc =
        last.gc === undefined ? sumGcCounts(performanceLogs, last.gc) : last.gc;
      var d = performanceLogs
        .filter(function(v) {
...
function sumGcCounts(traces, last) {
  ...
  return traces.reduce(function(prev, val) {
    ...
    ["V8.GCScavenger", "V8.GCIncrementalMarking", "MajorGC", "MinorGC"].forEach(
      function(v) {
        if (name === v) {
          if (params.ph === "B") {
...
function flow(set, driver) {
  var initialCounts;
  if (!driver) {
    throw new Error(driverErrorMessage);
  }
  ...
  executeInFlow(set.afterPrewarm).then(() => {
    getCounts(driver).then(data => (initialCounts = data));
  });
